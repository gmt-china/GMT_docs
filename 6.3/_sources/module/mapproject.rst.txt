...
**-C**\ [*dx*/*dy*]
   ...
   偏移量的单位是有效的绘图距离单位（请参阅 :term:`PROJ_LENGTH_UNIT`），但如果指定了\ ``-F``，偏移量单位为米。

.. _-D:

**-Dc**\|\ **i**\|\ **p**
   临时覆盖 :term:`PROJ_LENGTH_UNIT`，并改用\ ``c``\ (厘米)，``i``\ (英寸)或\ ``p``\ (像素)为单位。该选项
   不能与\ ``-F``\ 一起使用。
...
**-F**\ [*unit*]
   ...
   则输出（或输入，请参见\ ``-I``）以 :term:`PROJ_LENGTH_UNIT` 指定的单位（或者采用\ ``-D``）。
...
**-N**\ [**a**\|\ **c**\|\ **g**\|\ **m**]
   从大地纬度（使用当前的椭球，参阅 :term:`PROJ_ELLIPSOID`）转换为四个不同辅助纬度中的一个，经度不受影响。
...
**-Z**\ [*speed*][**+a**][**+i**][**+f**][**+t**\ *epoch*]
   ...
   速度为单位时间(单位由 :term:`TIME_UNIT` [m/s]指定)内通过的距离（单位由\ ``-G``\ 指定）。
   附加\ ``+i``\ 可以输出连续点之间的增量行进时间，``+a``\ 可以获取累积的行进时间，
   或者同时加上以获取两种时间信息。使用\ ``+f``\ 将累积的（经过的）旅行时间格式化为ISO 8601规定格式。
   至于用于表示秒的小数位数，请查阅 :term:`FORMAT_CLOCK_OUT`\ 设置。附加\ ``+t``\ epoch以报告连续点的绝对时间（ETA）。
...
在经纬度坐标下，计算点 (104.92, 29) 相对于点 (104.92, 31) 的方位角和大圆距离（单位为 km）。
下面的例子使用了 ``-je`` 选项以获得高精度的测地线距离::
