...
子部分（年、月、日、时、分、秒）以及 GEODAS id 以及权重（见 :option:`-W` ）等。通过指定沿轨的
...
.. list-table:: 测量数据字段说明
   ...
   * - **time**
     - 时间，从绝对时间（默认）、相对时间以及小数年时间中选择。三种时间的参数设置分别为：

       - 绝对时间：:term:`FORMAT_DATE_OUT` 和 :term:`FORMAT_CLOCK_OUT`
       - 相对时间：:term:`FORMAT_FLOAT_OUT` 和 :term:`TIME_SYSTEM` 或者（:term:`TIME_EPOCH` 和 :term:`TIME_UNIT`）
       - 小数年时间：:term:`FORMAT_FLOAT_OUT`
   * - **lon**
     - 经度，格式由 :term:`FORMAT_GEO_OUT` 指定
   * - **lat**
     - 纬度，格式由 :term:`FORMAT_GEO_OUT` 指定
   * - **twt**
     - 双程传播时间，单位 s
   * - **depth**
   ...
   * - **bcc**
     - 测深改正代码：
       ...
       - 01-55 Matthews zone 改正
       - 59 Matthews 改正，但未指定 zone
   ...
   * - **ngrav**
     - 国际重力参考场，即正常重力，单位 mgal：
       ...
       - 如果该参数未设置就使用 IGF80。也可以使用 **-Af** 指定
   * - **ceot**
     - | 从导航信息中计算 Eotvos 改正：
       | :math:`E = 7.5038 \cdot V \cdot \cos(lat) \cdot \sin(az) + 0.004154 \cdot V^2`


除上述列名外，下表中还列出了一些其他可识别的参数：
...
.. list-table:: 参数含义说明
   ...
   * - **geo**
     - 输出 10 个字段，分别为 **time**、**lon**、**lat** 以及 7 个地球物理观测字段：**twt**、**depth**、**mtf1**、**mtf2**、**mag**、**gobs** 和 **faa**
   * - **dat**
     - 与 **mgd77t** 相同，但使用 plain table（纯文本表格）文件顺序
   * - **all**
     - 返回文件中所有数据列
   * - **allt**
     - 与 **mgd77t** 相同，但时间项输出为“日期-时间”格式
...
是特殊的 Unix 字符，建议在 :option:`-F` 整个参数外添加引号。(3) 逻辑测试仅适用于观测数据，派生
数据，例如距离和速度等，必须使用 :option:`-D` ，:option:`-Q` ，:option:`-S` 等选项。
...
记录标志和用于沿轨的系统改正不同，后者不能使用 :option:`-T` 选项。
...
**-A**\ **c**\|\ **d**\|\ **f**\|\ **m**\|\ **t**\ *code*\ [**+f**]
    ...
    **faa** ），以及 Carter 深度改正（ **carter** ）如果被 :option:`-F` 选项选择的话，都会按原
    样输出；:option:`-A` 选项可改变该行为。对于上面的每种数据，都有 2-4 种方法来调整数据。通过
...
**-Af**
    ...
    **-Af1**\ [,\ *field*] 返回储存在数据集中的 **faa** [默认]。可使用 :option:`-F` 选项输出 **ngrav**
...
**-T**\ [**m**\|\ **e**]
    ...
    （仅限于 MGD77+ 文件）不被视为系统误差改正。相反，这些位标志可以通过 :option:`-F` 选项中的
    冒号来控制。
...
当通过 :option:`-A` 选项显式地或通过 MGD77+ 文件中 E77 元数据隐式地指定重新计算异常时，GMT 只计算
...
其中 *cruiseID* 为测线 ID，NCEI 定义的前缀，*observation* 为 :option:`-F` 选项里列名表中的某个列名，
