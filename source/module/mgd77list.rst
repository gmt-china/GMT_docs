...
还可能包含其他信息，见 :doc:`mgd77info` 的 **-C** 选项；若想添加自定义列，参考 
:doc:`mgd77manage` 。用户可以提取包括上面信息，8 个计算量（距离、航向、航向变化、速度、
Carter 改正、Eotvos 改正以及重力和磁的全球参考场）和时间的子单位中（年、月、日、时、分、
秒）以及 GEODAS id 以及权重（见 **-W** ）等任意信息的组合。通过指定沿轨的累积时间或距离
或者指定地理区域，可以选定特定的数据。提取出来的数据可以使用一定的逻辑判断来进一步筛选。
如果包括多个测线，则由段信息分隔。
...
**-F**\ *columns*\ [,\ *logic*][:\ *bittests*]
    ...
    否则，输出的 ASCII 文件将使用 :term:`FORMAT_FOLAT_OUT` 指定的格式。所有的
    可用的列名的缩写如下表：
...
+------------+-----------------------------------------------------------------------------+
| **time**   | 时间，从绝对时间（默认），相对时间，以及小数年时间中选择，其中三种时间的参数|
|            | 设置分别为：                                                                |
...
+------------+-----------------------------------------------------------------------------+
| **msens**  | 评估残余磁场的传感，1 第一个或者 leading 传感器，2 第二个或 trailing 传感   | 
|            | 器，9 未指定                                                                | 
+------------+-----------------------------------------------------------------------------+
...
**-Am**
    ...
    **-Amc**\ *offset* 使用由于测量位置和船位置不同（导航引起）导致的改正值。通过将总磁场
    ...
    :term:`GMT_INTERPOLAT` 设置的算法。*offset* 的默认单位为 m，可以通过 **f**，**k**，**M**
...
**-Db**\ *stopdate*
    不列出开始日期 *stopdata* 之前收集的数据，*stopdata* 格式为 yyyy-mm-dd\ **T**\ [hh:mm:ss]。
...
**-L**\ [*corrtable*]
    ...
    mgd77_corrections.txt 文件作为改正表]。改正表格式见 `改正`__
...
**-Qa**\ *min*/*max*
    指定可接收的方位角范围 *min*/*max* ，只输出该方位角范围内的记录 [0/360]。
**-Qc**\ *min*/*max*
...
*function* 可以是 sin，cos，或者 exp。可选的 *scale* 和 *origin* 用于将对参数进行一定的转换。
*abbrev* 参数是上面列出的列名之一。如果 *origin* 为 **T** ，则将其替换为文件中第一个 *abbrev*
值（同样同于时间）。如果第一条记录为 NaN，则将 *origin* 设置为 0。在乘以幅度前，可也对整个表达
式设置一定的幂指数 *power* 。
