...
并基于某些条件提取数据，输出结果可以为 ASCII 或二进制文件。
...
**-E**
    输出两个轨迹的名称和总的交叉点数据作为头部信息 [默认不写入这些头部信息]
...
**-F**\ *flags*
    ...
    **z** 沿轨观测值，见 **-C**
    ...
    在每列由两个输出，分别代表两个轨迹。否则，将只输出对应轨迹的结果
    （除 **n**,\ **N** ）。
...
**-W**\ [*list*]
    *list* 为本文文件，其中包含的信息为轨迹名称列表和用于计算交叉点处权重的
    相对权重。[默认权重设置为 1]
