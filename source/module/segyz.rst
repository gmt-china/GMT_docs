...
**segyz** 用于读取原生(IEEE)格式的 SEGY 文件，并绘制地震数据。使用 *imagemask* 操作可以将地震数据绘制成单色（用户可指定颜色或灰度）的 1 位深度位图，背景透明。位图分辨率采用当前 GMT 默认设置。地震道可以根据道头信息绘制在其实际位置上(这样文件中道的顺序就不再重要)。GMT 使用标准几何处理，因此理论上可以应用于任意地图投影，但使用地理投影可能会得到不理想的结果。需要注意的是，二维绘图器的某些选项在这里不可用。
