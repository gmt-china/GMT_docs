...
唯一的区别在于是否使用了 :option:`-S` 选项：

- 不使用 :option:`-S` 选项，默认将所有的数据点连成线，即绘制线段或多边形
- 使用 :option:`-S` 选项，则仅在数据点所在位置绘制符号
