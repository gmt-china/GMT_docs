...
**-F**\ [**R**\|\ **r**\|\ **h**\|\ **c**\|\ **x**][**+c**\ [*label*]][**+k**\ *keys*]
    ...
    一个文件，其中包含键值文件，也可以是单个字符，例如：D，则键值列表为 D、E、F...。
...
**-I**\ [**c**][**z**]
    - **-Ic** 选项翻转颜色的顺序，包括前景色和背景色以及:term:`COLOR_BACKGROUND` 和
      ...
      :doc:`/cpt/makecpt` ;
	- **-Iz** 选项翻转 CPT 的 *z* 值（不包括前景色、背景色）。该操作发生于 |-G| 和
      |-T| 选项之前，因此使用上述两选项时，需先考虑 **-Iz** 操作后的 *z* 值的实际
      范围。
...
**-N**
    不在生成的 CPT 中写入 前景色，背景色和 NaN 值的颜色[默认写入]。
...
使用离散 CPT 采样，则会警告，但仍可使用。GMT 这样做的原因是防止原始 CPT 出现混叠。
