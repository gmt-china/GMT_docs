...
**-A**\ [**m**\|\ **p**\|\ **x**\|\ **y**\|\ **r**\|\ **t**]
    ...
    默认情况下，地理线段以大圆弧的形式绘制。使用 :option:`-A` 将以直线绘制。
    ...
    注意：在 **plot3d** 中，:option:`-A` 选项要求 z 坐标为常数。

.. option:: -C

**-C**\ *cpt*
    提供一个 CPT 文件或使用 :option:`-C`\ *color1,color2*\ [*,color3*\ ,...] 来自动从这些颜色生成线性连续的 CPT。
    如果使用了 :option:`-S`，符号的填充颜色将由第四列的值决定。其他字段依次向后移动一列（符号大小将位于第 5 列而不是第 4 列，以此类推）。  
    如果未使用 :option:`-S`，则需要提供一个多段数据文件，其中每个段的头部包含 :option:`-Z`\ *value*，该值通过 CPT 控制线条或多边形（如果设置了 :option:`-L`）的颜色。 
...
**-G**\ *fill* 
    ...
    请注意，本命令会首先在输入数据文件的所有段头中搜索 :option:`-G` 和 :option:`-W` 字符串，任何找到的值都会覆盖本设置。  
    如果设置了 :option:`-Z`，可以使用 **-G+z** 通过 **-Ccpt** 和获取的 z 值指定填充颜色。  
...
**-W**\ [*pen*][*attr*] 
    ...
    * **+z** - 如果设置了 :option:`-Z`，则通过 :option:`-C`\ *cpt* 和获得的 *z* 值分配画笔颜色(如果通过 :option:`-Z` 设置透明度也是如此)。
...
**-Z**\ *value*\|\ *file*\ [**+t**\|\ **T**]
    ...
    要应用颜色，我们必须将 :option:`-G` 或 :option:`-W` 选项与 :option:`-Z` 结合使用：
