...
或者，传入 *image* ，它可以是一个图片文件（带地理参考或不带）。此时也可使用通过 |-I| 选项提供的文件进行光照。
这里，如果图片没有坐标，则使用强度文件的坐标。

使用地图投影时，网格首先会在具有相同尺寸的新矩形网格上重新采样。使用 |-E| 选项设置一个较大的 dpi 值，获得更高分辨率的图像。
...
因此当网格间距较大时，绘制出的图像会产生一定的失真与变形。 此时可以使用 |-E| 选项设置一个较大的 dpi 值，
将图像重采样成较高的分辨率缓解这一问题。 当然最根本的解决方式是使用精度更高的数据，
生成网格间距更小的 grd 网格文件进行绘图。

|-R| 选项可用于选择比网格范围更大或更小的地图区域。|-A| 允许直接输出到光栅文件，而不是通过 PostScript 绘图。

.. note::

    当前 GMT 6.5 版本 不支持 |-Q| 和 |-I| 同时使用，可切换到 GMT 6.4 或 GMT 开发版本，
...
**-A**\ *out_img*\ [**=**\ *driver*]
    ...
    **注：** 
    ...
    (2) 会丢失所有矢量元素。
    (3) |-B| 不能和 |-A| 同时使用，因为不输出 PostScript 。
...
**-D**\ [**r**]
    ...
    对于一些可能未知的图片格式你需要显式设置 |-D| ，以表明输入文件为图片格式，需用 GDAL 程序读取。
    后跟 **r** 表明范围由 |-R| 指定。例如，你使用了 **-Rd** 表明图片的全部范围。
    此模式允许你投影原始图片（即没有参考坐标的图片）。
...
**-M**
    使用 YIQ 转换（老式电视）强制将其转换为灰度图。不可与 |-Q| 共用。
...
**-Q**\ [*color*][**+i**][**+t**][**+z**\ *value*]
    ...
    + 网格： **-Q**\ [**+z**\ *value*]
      ...
      无参数的 |-Q| 将值为 NaN 的节点处设置为透明色，使用 PostScript Level 3 的 color-masking 功能
...
**-T**\ [**+o**\ [*pen*]][**+s**]
    ...
    此选项适用于值之间的插值没有意义的分类数据，并且通过 |-C| 提供了分类CPT。
...
请注意，如果您的输入网格中存在 NaN 区域，那么由于大多数地图投影所必须进行的重采样操作，
这些 NaN 块的范围可能会扩大。因为 **grdimage** 使用的是 PostScript 彩色图像操作符，
所以对于大多数非线性投影，我们必须将您的网格重采样到等距的矩形格网上。如果你
...
此时程序不知道该网格是否为分类网格，但如果通过 |-C| 提供的分类图谱是分类的，
程序将用 **-nn+a** （开启最近邻网格化并关闭抗锯齿）来覆盖您所选择的（可能是隐含的） **-n** 设置。
或者，可以使用 |-T| 来绘制每个节点为中心的单独多边形。

绘制分类图片
-----------------

如果给定的是一个单字节的单层图片，并且该文件没有颜色映射，那么我们将把字节值视为类别，
并且需要通过参数 |-C| 来指定一个分类的 CPT。如果没有给出参数 |-C| ，那么我们假定该图像是一个灰度图像，
其值在 0 到 255 的范围内。
...
然而，对于图像来说，这一点非常有限：我们可以选择图像的特定特征来表示透明度，例如特定的 *r*/*g*/*b* 颜色或 *alpha* 通道级别（0 - 255）。
因此，在复杂的 RGBA 图像（颜色 + 透明度）中，像素级别的可变透明度最多只能是单一颜色的可见形式。
我们的绘制透明的 RGBA 图像的近似方法是模拟颜色的透明效果，但图像本身仍然是不透明的（可通过 |-Q| 参数实现除单一颜色外的透明效果）。
由于多边形可以有不同的透明度，因此我们可以通过带有可单独设定颜色和透明度的正方形符号来模拟图像
（通过（最多）255 个值的 alpha 通道实现）。

识别图片格式
---------------

我们通过图片文件的特殊字节来自动识别图像格式。对于可能包含图片或数据集的格式（例如 GeoTIFF），
我们会判断其属于哪种类型，并相应地进行处理。如果您最喜欢的图像格式未被自动识别，
那么请告知我们它的特殊字节，以便我们添加支持。
...
快速绘制远程文件 *@AK_gulf_grav.nc* 并设置照明::
