...
**-D**\ [**r**]
    ...
    对于一些可能未知的图片格式用户需要显式设置 |-D| ，以表明输入文件为图片格式，需用 GDAL 程序读取。
    后跟 **r** 表明范围由 |-R| 指定。例如，用户使用了 **-Rd** 表明图片的全部范围。
...
并且需要通过参数 |-C| 来指定一个分类的 CPT。如果没有给出参数 |-C| ，那么 GMT 假定该图像是一个灰度图像，
其值在 0 到 255 的范围内。
...
GMT 绘制透明的 RGBA 图像的近似方法是模拟颜色的透明效果，但图像本身仍然是不透明的（可通过 |-Q| 参数实现除单一颜色外的透明效果）。
