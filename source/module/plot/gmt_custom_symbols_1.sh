GMT_DIR=$(dirname $(dirname $(which gmt)))
AWK=awk

grep -v '^#' "${GMT_DIR}"/share/custom/gmt_custom_symbols.conf | $AWK '{print $1}' > tt.lis
n=$(cat tt.lis | wc -l)
...
EOF
	...
	gmt begin GMT_App_N_$p png,pdf
...
rm tt.bars tt.lines tt.lis tt.symbols tt.text