...
**-S**\ [**~**]\ *"search string"*\|\ **+f**\|\ *file*\ [**+e**] \| |-S|\ [**~**]/\ *regexp*/[**i**][**+e**]
    ...
    *varname*\ =\ *value* 的形式提取数据段。 
    ...
    中，每行均为一个单独的字符串。
    **注** ：如果数据段的属性为多边形，如果多边形中间有孔，则输出时会同时输出孔。
...
**-T**\ [**h**][**d**\ [[**~**]\ *selection*]]
    ...
    - **d** 对于重复的数据，只输出一次。默认情况下，两条记录的所有列都相同才被
      认为是重复的数据，
    ...
    *start*\ [:*step*:]\ :*stop* 定义的一组列号，*step* 为步长，默认为 1。**~**
    用来反转选择。如果要考虑数据后的本文的比较，则在 *selection* 的最后添加 *t* ，
    如果不指定数字列，则只考虑最后的文本比较。

.. _-W:

**-W**\ [**+n**]
    将每个数据记录中的词转换为数字，并将这些数字追加到输出结果中。无法转换的
    词将设置为 NaN。使用 **+n** 可禁止输出有 NaN 的数据行。
