...
**-T**\ *now*
    ...
    如果 *now* 为绝对时间，则需要使用 :term:`TIME_UNIT` 来指示 :option:`-E` 和 |-L| 
    等选项中给出的值的单位。
...
**-Z**\ *command*
    ...
    *command* 命令部分必须以这些模块名称之一开头，
    ...
    不包括 :option:`-C`，|-G|，:option:`-J`，|-N|，:option:`-R` 和 |-W| （这些选项可能用于本模块）。
...
因此，如果 "time" 是一个单调递增的帧计数器，那么 :option:`-E` 和 |-L| 中的值就表示帧数。
当 "time" 实际上是通过日期/时钟标记给出的日历时间，会出现一种特殊情况。
在此情况下， :option:`-E` 和 |-L| 参数的单位由 :term:`TIME_UNIT` 设置决定[second]。
在解释用户的 :option:`-E` 和 |-L| 参数时，用户需要指定应使用的时间单位。
