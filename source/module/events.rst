...
:option:`-T`\ *now*
[ :option:`-A`\ **r**\ [*dpu*\ [**c**\|\ **i**][**+v**\ [*value*]]]\|\ **s** ]
[ :option:`-B`\ [**p**\|\ **s**]\ *parameters* ]
[ :option:`-C`\ *cpt* ]
[ :option:`-D`\ [**j**\|\ **J**]\ *dx*\ [/*dy*][**+v**\ [*pen*]] ]
[ :option:`-E`\ [**s**\|\ **t**\ ][**+o**\|\ **O**\ *dt*][**+r**\ [**c**\|\ **l**\|\ **q**]\ *dt*][**+p**\ *dt*][**+d**\ [**c**\|\ **l**\|\ **q**]\ *dt*][**+f**\ [**c**\|\ **l**\|\ **q**]\ *dt*][**+l**\ *dt*] ]
[ :option:`-F`\ [**+a**\ *angle*][**+f**\ *font*][**+j**\ *justify*][**+r**\ [*first*]\|\ **z**\ [*format*]] ]
[ :option:`-G`\ *color* ]
[ :option:`-H`\ *labelbox* ]
[ :option:`-L`\ [*length*\|\ **t**] ]
[ :option:`-M`\ **i**\|\ **s**\|\ **t**\|\ **v**\ *val1*\ [**+c**\ *val2*] ]
[ :option:`-N`\ [**c**\|\ **r**] ]
[ :option:`-Q`\ *prefix* ]
...
**-T**\ *now*
    设置当前绘图时间 *now*。在 :option:`-E` 中设置的时间偏移和增量都是相对于当前时间的。
    如果 *now* 为绝对时间，则需要使用 :term:`TIME_UNIT` 来指示 :option:`-E` 和 :option:`-L` 
    等选项中给出的值的单位。
...
**-A**\ **r**\ [*dpu*\ [**c**\|\ **i**][**+v**\ [*value*]]]\|\ **s**
    当未使用 :option:`-S` 时，**events** 模块将读取直线或多边形。支持两种不同的输入形式，
    并通过所选的 :option:`-A` 指令指定：

    + **-Ar** - 输入数据为轨迹线（详见 `绘制轨迹线`_ ），输入数据格式为 *x, y, time* ，
      这里，“事件”指受当前时间（*now*）、事件持续时间（:option:`-L`）
      和上升/淡出周期（**-Es**）限制的轨迹的一部分。
      如果当前时间落在轨迹上的两点之间，那么使用线性插值来找到当前的终点。

    + **-As** - 输入数据为多个多边形或线条，输入数据格式为 *x, y* 。
      **events** 模块读取每段记录，并通过段头中的 :option:`-T`\ *string* 获取每段的显示时间 *time*。
      这里，“事件”指受当前时间（*now*）、事件持续时间（:option:`-L`）
      和上升/淡出周期（**-Es**）限制的整个多边形或线条。
      如果使用了 :option:`-L`\ [**t**] ，则 *string* 的格式必须是 
    ...
    *x, y*\ [, *zcols* ]、*time* 数据转换为 *密集采样的点*，
    ...
    同时还能利用 **-Es** 和 **-Et** 的完整机制以及由 :option:`-M` 控制的所有符号效果。
...
**-F**\ [**+a**\ *angle*][**+f**\ *font*][**+j**\ *justify*][**+r**\ [*first*]\|\ **z**\ [*format*]]
    ...
    - **+z**\ [*format*] - 将传入的 z 值格式化作为文本标签 (要求使用 :option:`-C` ，可自定义格式 *format* [:term:`FORMAT_FLOAT_MAP`]).
...
**-L**\ [*length*\|\ **t**]
    ...
    如果未给出 :option:`-L` 参数，则假定事件具有无限的持续时间。
    有关线条和多边形的持续时间处理方式，请参阅 :option:`-A` 。
...
**-M**\ **i**\|\ **s**\|\ **t**\|\ **v**\ *val1*\ [**+c**\ *val2*]
    ...
    这意味着除非用户通过一个或多个 **+c** 修饰符（每个 :option:`-M` 指令一个）来更改这些属性，
    否则在尾声期间符号不会被绘制出来。
...
**-S**\ *symbol*\ [*size*]
    指定用于该事件的符号（默认为绘制线条或多边形；请参阅 :option:`-A` ）。
    视需要，将符号大小与单位 (**c|i|p**) 一起添加。
    如果没有给出大小，则会从数据文件的第三列（如果使用 :option:`-C` 则为第四列）
    读取事件特定的大小。 **注：** 并非 :doc:`plot` 中所有符号都能在此处使用。
    目前本模块仅支持基本符号和自定义符号；
    不支持 bars, vectors, ellipses, fronts, decorated and quoted lines。
    从数据文件读取的符号大小的单位将假定为由 :term:`PROJ_LENGTH_UNIT` 控制，除非指定了单位 (**c|i|p**) 。如果只是绘制文本标签，则不需要 :option:`-S` 。
...
**-Z**\ *command*
    ...
    *command* 命令部分必须以这些模块名称之一开头，
    ...
    :option:`-Z`\ "coupe -Q -L -Sc3c -Ab128/11/120/250/90/400/0/100+f -Fa0.1i/cc"。
...
通常情况下，"time"（例如在 :option:`-T` 参数的输入中）的含义实际上是指所绘制现象的时间，
但它也可以是任何其他便于使用的单调递增量，比如运行时间、模型时间、帧数或类似距离的其他物理量。
这完全取决于动画的上下文。在 :option:`-E` 中给出的下降、淡出等值，
均假定与 :option:`-T` 参数给出的 "time" 参数使用相同的单位。
因此，如果 "time" 是一个单调递增的帧计数器，那么 :option:`-E` 和 :option:`-L` 中的值就表示帧数。
当 "time" 实际上是通过日期/时钟标记给出的日历时间，会出现一种特殊情况。
在此情况下， :option:`-E` 和 :option:`-L` 参数的单位由 :term:`TIME_UNIT` 设置决定[second]。
在解释用户的 :option:`-E` 和 :option:`-L` 参数时，用户需要指定应使用的时间单位。
...
4. 最后，用户无需为符号指定固定颜色，而是可以使用 :option:`-C` 选项来设置一个颜色表，
...
如以上示例所示，上升、平台、淡出和尾声周期都是可选的，可以根据需要通过 :option:`-E` 选择或忽略。
...
虽然只会在通过 :option:`-T` 设置的给定时间绘制应该可见的事件，但绘制的顺序仅由文件中记录的顺序给出。
因此，如果文件没有按升序时间排序，那么后面的事件可能会位于前面的事件之下。
为了防止这种情况，用户可以通过 :doc:`convert` **-N** 将文件按升序排序。
