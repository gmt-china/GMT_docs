...
**-M**\ *max_radius*
    ...
    **-M1c** 将保留处其所处格点外的最近一圈格点，即 3x3 的范围，依次类推。

.. _-N:

**-N**\ *max_iterations*
    迭代次数, 默认为 501。迭代将会在达到收敛极限或迭代次数达到 *max_iterations* 时停止。这个
    最大迭代次数仅针对最终输出的网格，对于 GMT 内部的临时网格没，其迭代次数放大为
    网格间隔相应的倍数。
