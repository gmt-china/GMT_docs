...
一般情况下，需要使用meca绘制震源球，再使用polar将每个台站的极性信息画在
相应的震源球上。因而，通常meca和polar需要连在一起使用。
...
``-M<size>``
    震源球的尺寸，需要与meca中 ``-S`` 选项中震源球尺寸相同

``-S<symbol_type><size>``
    类似于plot的 ``-S`` 选项，控制极性要以什么符号以及多大尺寸绘制到震源球上，可选的符号类型包括 ``a|c|d|h|i|p|s|t|x``
