...
高度；接下来两列为每个棱柱的增量 *dx* 和 *dy* ，见 :option:`-E` ；最后一列为
棱柱的密度，如果 :option:`-D` 指定了密度，则被固定密度覆盖。或者，也可以使用
:option:`-C` 选项来创建棱柱。如果单个棱柱密度可变（ :option:`-H` ），则每个棱柱被分解为
密度恒定，堆叠的子棱柱，垂直增量为 *dz* 。该模块可以计算等距网格上的异常
（通过 :option:`-R` 和 :option:`-I` 选项指定）或通过 :option:`-N` 指定任意的计算点。可计算的异常
...
上图展示了三种不同密度的海山，海山都由棱柱构成。左图为常密度（:option:`-D`）；
中图为垂向等密度（:option:`-D`）；右图为随 *r* 和 *z* 变化的密度（:option:`-H`）。
...
[ :option:`-F`\ **f**\|\ **n**\ [*lat*]\|\ **v** ]
[ :option:`-G`\ *outfile* ]
[ :option:`-H`\ *H*/*rho_l*/*rho_h*\ [**+d**\ *densify*][**+p**\ *power*] ]
[ :option:`-I`\ *increment* ]
[ :option:`-L`\ *base* ]
[ :option:`-M`\ [**h**]\ [**v**] ]
[ :option:`-N`\ *trackfile* ]
...
**-C**\ [**+q**][**+w**\ *file*][**+z**\ *dz*] ]
    与其他选项组合创建棱柱，1）与 :option:`-S`\ *height* 组合创建棱柱；2）与
    :option:`-L`\ *base* 和 :option:`-T`\ *top* 指定的两个面创建棱柱。对于第二种情况，
    *base* 和 *top* 可以为常数或者网格。对于第一种情况，则创建从 0 到
    *height* 的棱柱。如果使用 :option:`-H` 设置可变密度，则将每个棱柱分为多个堆叠
    的具有单独密度的子棱柱，因此需要为这些子棱柱设置高度增量 *dz* ，第一个
    和最后一个棱柱可能会对高度进行适当的调节。如果不使用 :option:`-H` 选项，则只
    创建统一密度的棱柱，通过 :option:`-D` 可以使每个棱柱具有不同的密度。
...
**-D**\ *density*
    ...
    此时，需要 :option:`-C` 选项，并必须和 :option:`-S` （或 :option:`-L` 和 :option:`-T` ）提供的网格具有
    相同的大小和配准方式。
...
**-E**\ *dx*\ [/*dy*]
    ...
    如果 :option:`-E` 选项只给出 *dx* ，则 *dy* = *dx* 。对于地理坐标，*dx* 是经度
...
**-F**\ **f**\|\ **n**\ [*lat*]\|\ **v**
    ...
    - **n** 大地水准面，可指定一个平均纬度，默认使用网格或者 :option:`-N` 指定的
      纬度的中值
...
**-S**\ *height*
    指定完整的海山网格名称，用于创建棱柱或者 :option:`-H` 所需
...
**-W**\ *avedens*
    指定由 :option:`-C` 和 :option:`-H` 创建的随空间变化的密度网格名，作为输出
