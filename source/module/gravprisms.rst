...
:option:`-C` 选项来创建棱柱。如果单个棱柱密度可变（ |-H| ），则每个棱柱被分解为
密度恒定，堆叠的子棱柱，垂直增量为 *dz* 。该模块可以计算等距网格上的异常
（通过 :option:`-R` 和 |-I| 选项指定）或通过 |-N| 指定任意的计算点。可计算的异常
...
中图为垂向等密度（:option:`-D`）；右图为随 *r* 和 *z* 变化的密度（|-H|）。
...
**-D**\ *density*
    ...
    此时，需要 :option:`-C` 选项，并必须和 |-S| （或 |-L| 和 |-T| ）提供的网格具有
    相同的大小和配准方式。
...
**-W**\ *avedens*
    指定由 :option:`-C` 和 |-H| 创建的随空间变化的密度网格名，作为输出
