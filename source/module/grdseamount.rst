...
位置和大小，即 *lon*, *lat*, *radius*, *height* 。对于椭圆形的海山（ :option:`-E`
），则包括 *lon*, *lat*, *azimuth*, *semi-major*, *semi-minor*, *height* 。
如果使用 :option:`-F` 选项以生成平顶海山，但后面没有 *flattening* 参数，则输入数据的
最后一列为 *flattening* 参数。对于随时间变化的地形，可以使用 :option:`-T` 选项，这样
输入数据的最后两列为海山的开始和停止时间。这种情况下，可以输出海山最终的累积
形状，也可以输出其中每个时间点的形状或变化（ :option:`-Q` ）。如果海山存在滑坡（ :option:`-S` ），
则输入数据中最后一列或者几列为滑坡参数。
...
[ :option:`-E` ]
[ :option:`-F`\ [*flattening*] ]
[ :option:`-H`\ *H*/*rho_l*/*rho_h*\ [**+d**\ *densify*][**+b**\ *boost*][**+p**\ *power*] ]
[ :option:`-K`\ [*densitymodel*] ]
[ :option:`-L`\ [*hn*] ]
[ :option:`-M`\ [*list*] ]
[ :option:`-N`\ *norm* ]
[ :option:`-Q`\ *bmode*\ [/*fmode*]\ [**+d**] ]
...
:math:`h_n` 是相对于 :option:`-L` [0] 指定的噪声基底的高度，*f* 是 :option:`-F` 选项设置的
...
**-H**\ *H*/*rho_l*/*rho_h*\ [**+d**\ *densify*][**+p**\ *power*]
    ...
    *power* ，即下图中的 :math:`\Delta \rho_f` 和 *p* ，默认 *power* 为 1，即为线性
    ...
    内部。如果海山为平顶海山（见 :option:`-F` ），*h(r)* 仍然为未截断的情况。
    **注** ：如果使用 :option:`-W` 选项，将会报告每个海山的平均密度。径向密度函数的定义为
...
**-Z**\ *level*
    ...
    可以将海山放在海洋里。可以追加 NaN 可以标记不被使用的节点（不能与 **-Qi**
    同时使用)。
...
通过 :option:`-S` 选项来模拟滑坡并不是物理模型，而是通过简单的几个形状和函数来
...
创建位于 1 分网格里在 1 W，2 S 处，基底半径为 30 km，高度为 4500 km 的圆形高斯海山::
...
2.8 Ma 到 1.9 Ma，海山体积变化率为常数；从 3 Ma 到 1.9 Ma 平均每 0.1 Myr 输出一个
网格 ::
    ...
    EOF
    gmt grdseamount -R-1024/1022/-1122/924+uk -I2000 -Gsmt_%3.1f_%s.nc t.txt \
      -T3M/1.9M/0.1M -Qi/c -Dk -E -F0.2 -Cg -Mfiles.txt
