...
:官方文档: :doc:`gmt:supplements/grd2kml`  
:简介: 从单个网格创建 KML 图像四叉树


**grd2kml** 读取一个二维网格文件，并使用所选的图块大小生成 PNG 或 JPG 图像
以及适用于 Google Earth 的 KML 文件 *四叉树（quadtree）*。
我们根据四叉树的不同视图层级，对网格进行高斯滤波降采样（可通过选项选择其他滤波器）。
可选地，通过提供 ±1 范围内的强度网格文件，或自动从输入网格派生强度（见 |-I|），可添加光照效果。
超出 ±1 范围的强度值将被裁剪。地图颜色通过色标（CPT）查表定义，可选择是否叠加等值线。
...
**-F**\ *filtercode*
    指定用于网格降采样的滤波器。
    可选：**b**（Boxcar 方窗）、**c**（余弦弓形）、**g**（高斯）、**m**（中值）[默认：Gaussian]。
    滤波宽度会自动随四叉树层级调整。
