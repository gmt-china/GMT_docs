...
#. 用 :option:`-C` 选项定义测线的中心，并用 :option:`-T` 选项定义rotation pole的位置

在指定测线后，如果要生成测线，输出测线上的点的坐标，可以使用 :option:`-G` 选项。
...
**-F**\ *flags*
    ...
    默认输出格式为 ``xyzpqrs`` 。若使用 :option:`-G` 选项，则输出格式为 ``rsp`` ，并且此时 :option:`-F` 选项无效。
...
**-G**\ *dist*\ [*unit*][/*colat*][**+n**]
    ...
    - 不使用 :option:`-Q` 选项时， *unit* 的设置无效。 *p* 和 *dist* 的单位强制为弧度。
    - 而使用 :option:`-Q` 选项时， *p* 和 *dist* 的单位默认为km。可以设置 *unit* 为其他长度单位，例如 **e** 表示为m。
...
**-L**\ [**w**\|\ *lmin*/*lmax*]
    ...
    *lmin* 和 *lmax* 的单位规定见 :option:`-Q` 选项。
    如果是负数则表示反方向的数据范围。
...
**-W**\ *wmin*/*wmax*
    仅坐标 *q* 在 *wmin* 和 *wmax* 范围内的数据才会被投影到测线上。单位规定见 :option:`-Q` 选项。
