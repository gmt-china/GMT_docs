...
**-A**\ [**f**\|\ **p**\|\ **m**\|\ **r**\|\ **R**][**+l**]
    关于沿测线进行采样（如果设置了 :option:`-C` 或 |-E| ）的一些选项，在 :option:`-A` 后追加：
...
**-C**\ *length*/\ *ds*\ [*/spacing*][**+a**\|\ **v**][**d**\|\ **f**\ *value*][**l**\|\ **r**] 
    ...
    **注：** 
    ...
    + 如果使用了 :option:`-C` 且设置了 *spacing* ，则 |-E| 设置的测线采样间隔将被覆盖。
    + 在地理坐标下， :option:`-C` 和 |-E| 使用不同单位会存在bug
...
**-T**\ [*radius*][**+e**\|\ **p**]
    用于常规网格采样过程（即读取 *table* 表数据，不使用 :option:`-C` 和 |-E| ），仅限于单个网格文件。
