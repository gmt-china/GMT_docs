...
**-Ab**\|\ **B**\|\ **f**\|\ **F**\|\ **o**\|\ **O**\ [*lon0*/*lat0*][**+v**]
    ...
    第二个点 (lon0/lat0)。有关 :option:`-A` 如何影响输出记录，请参见 `<输出顺序>`_ 。如果
    指定了 :option:`-R` 和 :option:`-J` 选项，则先进行投影，然后再计算投影后笛卡尔坐标下的方位角。
...
**-C**\ [*dx*/*dy*][**+m**]
   ...
   投影中心在东方向和北方向的
   偏移量，投影结果将加上该偏移量，详见 `示例`_ 中的高斯投影和 UTM 投影；如果和 :option:`-I` 选项
   同时使用，则先减去该偏移量，然后再进行逆投影变换。
   偏移量的单位是有效的绘图距离单位（请参阅 :term:`PROJ_LENGTH_UNIT` ），但如果指定了 :option:`-F`
...
**-F**\ [**e**\|\ **f**\|\ **k**\|\ **M**\|\ **n**\|\ **u**\|\ **c**\|\ **i**\|\ **p**]
   ...
   如果要指定其他单位，请附加所需的单位（请参阅 :doc:`/basis/unit` ）。如果没有 :option:`-F` ，
   则输出（或输入，请参见 :option:`-I` ）使用 :term:`PROJ_LENGTH_UNIT` 指定的单位（参见 :option:`-D` ）。
...
**-L**\ *table*\ [**+p**][**+u**\ *unit*\|\ *c*\|\ *C*]
    ...
    例如，其中 *unit* 也可使用如下字符，**c** 表示使用输入坐标的笛卡尔距离，**C**
    表示使用投影坐标的笛卡尔距离，**C** 需要设置 :option:`-R` 和 :option:`-J` 。
...
**-W**\ [**e**\|\ **E**\|\ **g**\|\ **h**\|\ **j**\|\ **n**\|\ **o**\|\ **O**\|\ **r**\|\ **R**\|\ **w**\|\ **x**][**+n**\ [*nx*\ [/*ny*]]]
    ...
    输出的绘图的维度的单位可通过 :option:`-D` 修改。

    - **w** 输出地图高度
    - **h** 输出地图宽度
    - **g**\ *lon*/*lat* 输出 *lon*/*lat* 对应的绘图坐标
    - **j**\ *code* 输出参考点(绘图坐标)对应的地图坐标，参考点请见 :doc:`/basis/anchor`
    - **r** 在使用 :option:`-R` 和 :option:`-J` 定义倾斜区域时，输出其覆盖的矩形范围
    - **R** 与 **r** 相同，但输出 -Rw/e/s/n 形式
    - **o** 在使用 :option:`-R`\ *xmin/xmax/ymin/ymax*\ **+u**\ *unit* 将地理坐标投影为笛卡尔坐标时，
      返回对角点地图坐标，单位为度，输出顺序为 llx urx lly ury
    - **O** 与 **o** 相同，但以 :option:`-R` 的形式输出
    - **e** 返回由 :option:`-R` 和 :option:`-J` 定义的非矩形区域覆盖的矩形区域
    - **E** 与 **e** 相同，但以 :option:`-R` 的形式输出
