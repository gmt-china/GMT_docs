...
#. 地图投影，读取经纬度数据，并使用指定的地图投影和比例尺计算投影坐 (x,y)
...
**-Ab**\|\ **B**\|\ **f**\|\ **F**\|\ **o**\|\ **O**\ [*lon0*/*lat0*][**+v**]
    ...
    第二个点 (lon0/lat0)。有关 |-A| 如何影响输出记录，请参见 `输出顺序>` 。如果
...
**-F**\ [**e**\|\ **f**\|\ **k**\|\ **M**\|\ **n**\|\ **u**\|\ **c**\|\ **i**\|\ **p**]
   ...
   如果要指定其他单位，请附加所需的单位（请参阅 `units`_ ）。如果没有 **-F** ，
...
**-G**\ [*lon0*/*lat0*][**+a**][**+i**][**+u**\ *unit*][**+v**]
    ...
    指定单位及距离计算方式，参见 `units`_ ，默认使用大圆距离。
...
**-L**\ *table*\ [**+p**][**+u**\ *unit*\|\ *c*\|\ *C*]
    ...
    追加 **+u** 可指定距离的单位和距离计算方式，见 `units`_ ，默认使用大圆距离。
    例如，其中 *unit* 也可使用如下字符，**c** 表示使用输入坐标的笛卡尔距离，**C**
...
**-T**\ [**h**]\ *from*\ [/*to*]
   ...
   基准可通过基准 ID（参见 **-Qd**）指定或给通过 *ellipsoid*:*dx*,\ *dy*,\ *dz
...
**-W**\ [**e**\|\ **E**\|\ **g**\|\ **h**\|\ **j**\|\ **n**\|\ **o**\|\ **O**\|\ **r**\|\ **R**\|\ **w**\|\ **x**][**+n**\ [*nx*\ [/*ny*]]]
    ...
    - **j**\ *code* 输出参考点(绘图坐标)对应的地图坐标，参考点请见 :doc:`basis/embellishment`
