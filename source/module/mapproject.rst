...
**-Ab**\|\ **B**\|\ **f**\|\ **F**\|\ **o**\|\ **O**\ [*lon0*/*lat0*][**+v**]
    ...
    指定了 |-R| 和 :option:`-J` 选项，则先进行投影，然后再计算投影后笛卡尔坐标下的方位角。
...
**-G**\ [*lon0*/*lat0*][**+a**][**+i**][**+u**\ *unit*][**+v**]
    ...
    表示使用投影坐标的笛卡尔距离，**C** 需要设置 |-R| 和 :option:`-J` 。如果没有给出
...
**-L**\ *table*\ [**+p**][**+u**\ *unit*\|\ *c*\|\ *C*]
    ...
    例如，其中 *unit* 也可使用如下字符，**c** 表示使用输入坐标的笛卡尔距离，**C**
    表示使用投影坐标的笛卡尔距离，**C** 需要设置 |-R| 和 :option:`-J` 。
...
**-T**\ [**h**]\ *from*\ [/*to*]
   ...
   基准，则假定为 WGS84。**-T** 和 |-R| 以及 :option:`-J` 同时使用时，会先做基准转换，然后
