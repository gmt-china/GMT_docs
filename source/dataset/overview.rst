...
对于一些公开的地学数据，GMT对数据处理成不同分辨率不同配置方式的数据，并保存
到GMT的数据服务器上。当用户\ **第一次**\ 使用某个分辨率的某个数据时，
GMT会自动从服务器上下载该数据文件，并保存到GMT用户目录下的特定子目录下，
（默认为 **~/.gmt/server** 目录的子目录），然后再读取该文件。
当用户再次该数据时，GMT会自动从本地GMT用户目录下读取该数据文件，
而无需再次从服务器下载。

数据分辨率
++++++++++

通常，原始数据提供者只提供单个、高精度的数据格式，但有些时候用户只需要低精度的
数据即可满足需求。因而GMT通过对原始数据做笛卡尔高斯滤波以进行减采样，提供了多种
不同分辨率以及不同配准方式的数据。
...
为了解决这一问题，对于高分辨率的数据，GMT将其分割成若干个小区块，
...
下载该区域几MB的网格数据，而无需下载整个7 GB的3弧秒，极大的节省了下载时间。

数据压缩
++++++++

GMT以无损且高压缩率的netCDF-4短整型格式保存单个网格数据，使得GMT提供的数据比
原始数据占据更小的硬盘空间，而不丢失任何精度信息。对于分区块的高分辨率数据，
...
当原始数据发布了新版本，或GMT提供的数据存在问题时，GMT会对数据服务器上的数据
...
#. 当再次使用任一远程数据时，GMT会检查本地文件 **gmt_data_server.txt** 的日期，
   若其日期比当前时间差一天（该日期可修改）以上，则再次从GMT服务器下载该文件
   **gmt_data_server.txt** 并替换本地的老数据
...
对于分区块的高精度数据，如果想要将某个区域的数据以单个文件的形式保存，并供其它
