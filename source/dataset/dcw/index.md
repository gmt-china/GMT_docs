...
1. 在维基百科页面 [ISO Country Codes](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) 中查找

2. 使用命令 `gmt coast -E+l` 查看国家代码列表

3. 从DCW辅助文件 {download}`dcw-countries.txt` 中查找，其文件格式为:
...
1. {doc}`-R 选项 </option/R>` 中可以直接使用区域代码以间接指定绘图范围
2. {doc}`/module/coast` 模块 **-E***code1*,*code2*,... 选项调用 DCW 数据绘制或导出国界/省界
