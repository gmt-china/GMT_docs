...
直接使用安装包安装 GMT 可能无法正常运行使用。用户在使用安装包之前需要对系统环境进行如下检查。

打开命令提示符 cmd，输入以下命令检查你的 Windows 登录用户名：
...
GMT 只支持用户名为字母、数字、下划线的组合。如果用户名包含空格、中文或其他符号，将导致 GMT 运行出错。
如果用户名不符合要求，请新建一个用户，然后使用新用户登录 Windows。
...
2. 安装 GMT
   ...
   解决办法是先忽略这一警告，待安装完成后按照上面的步骤手动修改系统环境变量 **PATH**。
...
4. 测试安装
   ...
   用户需要按照上面的步骤手动修改系统环境变量 **PATH**，添加 GMT 安装目录下的 `bin` 目录的路径。
