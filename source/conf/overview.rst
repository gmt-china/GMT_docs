...
.. note::
    ...
    第一个命令 ``plot`` 绘制了一张图，第二个命令 ``set`` 修改了GMT配置参数，
    例如将默认字体A修改为字体B，第三个命令 ``plot`` 绘制了第二张图。此时，
    第一个命令 ``plot`` 用的是默认字体A，而第三个命令 ``plot`` 用的是字体B。

    当执行完一次该脚本时，在当前目录下会生成配置文件 :file:`gmt.conf`\ ，其中的
    配置参数使用的是字体B。

    若再次执行该脚本，由于当前目录下已经有了上一次执行生成的 :file:`gmt.conf`
    文件，且文件中配置参数使用的是字体B，则第一个命令 ``plot`` 受到该参数文件
    影响而使用了字体B，而第二个命令 ``set`` 将字体B修改为字体B（相当于没修改），
    第三个命令 ``plot`` 则使用字体B。这导致了执行同一个脚本出现了不同的结果，且
    难以调试和拍错。
