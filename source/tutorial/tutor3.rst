...
#. ``grdcut`` 指从大区域地形中裁剪出自定义范围的数据（本例中为55-120°E，10-50°N），
...
#. ``grdimage`` 模块中： ``-I`` 接 ``grdgradient`` 计算得到的梯度文件， ``-C``
   接地形渲染颜色表文件。``-I`` 若未指定文件，则自动调用 ``grdgradient`` 计算梯度。
...
#. ``colorbar`` 模块绘制图中左侧的两个色标。两个色标均绘制了颜色表 ``Tibet.cpt`` 的
   ...
   ``-Dx-0.3i/1.4i`` 表示第一个色标绘在左下角原点左移0.3inch，上移1.4inch的位置，
   ``+w2.5i/0.15i`` 表示色标高2.5inch，宽0.15inch， ``+ma`` 表示色标的文字标注位于左侧。