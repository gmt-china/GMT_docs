...
要绘制全球地图，就需要将三维地球投影到一张二维纸面上，因而需要指定投影方式。
...
.. gmtplot::
    ...
    gmt coast -JM12c -RJP -Ba -W0.5p -A10000 -png JapanMap

这里我们使用了：

-   **-JM12c** 指定了使用墨卡托投影（绘制区域地图最常用的投影方式），地图宽度为12厘米；
-   **-RJP+r1** 指定了绘图区域范围，此处 **JP** 是日本的国家代码，GMT会自动根据国家代码
    提取该国家的区域范围并用于绘图；
-   **-Ba** 用于给底图添加边框。对于某些地图投影，GMT默认使用图中所示的黑白相间的
    底图边框。\ **a** 用于添加标注（\ **a**\ nnotation，即图中的经纬度数字）。

从图中可以看到，绘图区域的北边和东边与日本的最北端和最东端吻合，但绘图区域的南边
和西边似乎远远超过了日本的最南端和最西段。是不是GMT提取国家范围时出错了呢？
其实不是，是因为日本拥有很多小的岛屿。在计算国家对应的区域范围时，这些岛屿也被
考虑在内；而我们在绘图的时候没有绘制这些面积较小的岛屿（后面会介绍到如何控制
是否绘制这些小岛屿），因而存在上图中看到的“不完美”的情况。

解决办法也很简单，我们可以使用 **-R** 选项最原始的方式来指定绘图区域范围，
即依次给定绘图区域的最小经度、最大经度、最小纬度和最大纬度。记起来也很容易，
可以记为 *xmin/xmax/ymin/ymax*\ ，或者直接记“西东南北”。

通过上面“不完美”的图，我们大概知道了日本的经纬度范围。因而可以直接使用
**-R128/146/30/46** 指定绘图区域。

.. gmtplot::
    :caption: 使用经纬度范围指定绘图区域
    :width: 50%

    gmt coast -JM12c -R128/146/30/46 -Ba -W0.5p -A10000 -png JapanMap
...
-   **-BWSen** 中WSEN分别是西南东北四个方向的英文单词首字母，用于控制四条边的属性。
    ...
    只绘制边框但不显示标注
-   **-BWSen+t"Japan Map"** 中 **+t** 用于给整张图添加标题（\ **t**\ itle）。
    由于标题 Japan Map 中含有空格，我们需要使用单引号或双引号将其括起来

.. gmtplot::
    :caption: 为区域地图添加标题
    :width: 50%

    gmt coast -JM12c -R128/146/30/46 -Bxa4 -Bya3 -BWSen+t"Japan Map" -W0.5p -A10000 -png JapanMap
