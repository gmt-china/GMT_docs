...
#. 除了上一步的 ``coast`` 命令绘制底图之外，又加了几个 ``plot`` 命令。
   因为行数变多，所以我们通常会定义使用脚本
...
``plot`` 模块除了绘制符号，也绘制线条。下面我们用 ``plot`` 给图画加上射线路径。
...
一条GMT绘图命令只绘制了一个图层，图层具有叠加的属性，后面绘制的图层会覆盖前面
绘制的图层，简单的说就是后来者居上。
