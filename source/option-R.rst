...
#. ``-R<code><x0>/<y0>/<nx>/<ny>``
   ...
   该方法仅可在创建网格文件时使用。 ``<code>`` 用于指定网格区域的 :doc:`anchors` 。 ``<code>`` 所指定的锚点对应的坐标为 ``(x0, y0)`` ， ``<nx>`` 、 ``<ny>`` 是X和Y方向的网格数，通过 ``-I`` 选项指定的网格间隔即可计算得到区域范围。
...
默认为 ``yyyymmddThh:mm:ss.xxx`` 格式，见 :doc:`format` 一节。
...
既可以用浮点数表示也可以用指数表示，见 :doc:`format` 一节。
